...
3. **Visitation modeling**
   - Build per‑resort baseline: mean weekly visitation by `Week` index across historical seasons (optionally smoothed).
   - Estimate school‑holiday uplift: multiplicative factor learned from historical weeks overlapping VIC/NSW Term‑2 holidays.
   - Add gentle trend term if needed (per‑resort linear year trend or season fixed effects).
   - 2026 weekly forecast per resort: `forecast_week = baseline_week × holiday_multiplier(week) × trend_adjustment(2026)`.
   - Normalize forecast within resort (z‑scores) to form the crowd score.
   - For 7‑day windows spanning multiple weeks, blend the weekly crowd/price proxies by day‑overlap weights.
4. **Scoring framework**
...
- **Visitor forecasting (weekly, per resort)**
  ...
  - Holiday uplift validation: inspect VIC/NSW coefficients; permutation test by shuffling holiday flags to confirm uplift isn’t spurious.
  - Baselines: compare against week‑mean and multiplicative baseline; keep the simplest model that meets accuracy.
  - Uncertainty: residual bootstrap to derive prediction intervals; propagate to z‑scores for composite.
